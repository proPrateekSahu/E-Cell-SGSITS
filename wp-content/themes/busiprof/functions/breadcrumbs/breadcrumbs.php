...
function busiprof_custom_breadcrumbs() {
  ...
  $home = esc_html__('Home','busiprof'); // text for the 'Home' link
  ...
  elseif (is_home() || is_front_page()) {
    ...
    if ( is_category() ) {
      ...
      echo $before . esc_html__('Archive by category','busiprof'). ' "' . single_cat_title('', false) . '"' . $after;
 
    } elseif ( is_search() ) {
      echo $before . esc_html__('Search results for','busiprof').' "' . get_search_query() . '"' . $after;
  ...
  elseif ( is_attachment() ) {
    ...
    $cat = $cat[0];
      ...
      echo $before . esc_html__('Posts tagged','busiprof').' "' . single_tag_title('', false) . '"' . $after;
 
    } elseif ( is_author() ) {
       global $author;
      $userdata = get_userdata($author);
      echo $before . esc_html__('Articles posted by','busiprof').' ' . $userdata->display_name . $after;
 
    } elseif ( is_404() ) {
      echo $before . esc_html__('Error 404','busiprof') . $after;
    }
 
    if ( get_query_var('paged') ) {
      if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
      echo ' ( ' . esc_html__('Page','busiprof') . '' . get_query_var('paged'). ' )';
      if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
